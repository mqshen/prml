我们已经把分类问题分成了隔开的两个阶段：使用训练数据学习$$ p(C_k|x) $$的模型的推断阶段（inference stage），和随后的使用这些后验概率来对类别作最优的分类的决策阶段（decision stage）。另一种可能是简单的学习一个函数来把输入$$ x $$直接映射成决策，来同事解决这两个问题。这样的函数被称为判别函数（discriminant function）。    
