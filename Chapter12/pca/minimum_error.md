...
其中，系数$$ \alpha_{ni} $$对于不同的数据点来说是不同的。这对应于将坐标系旋转到了一个由$$ \{u_i\} $$定义的新坐标系，原始的$$ D $$个分量$$ \{x_{n1|,...,x_{nD}\} $$被替换为一个等价的集合$$ \{\alpha_{n1},...,\alpha_{nD}\} $$。与$$ u_j $$做内积，然后使用单位正交性质，我们有$$ \alpha_{nj} = x_^Tu_j $$，因此不失一般性，我们有    
