...
$$的每个列上定义一个独立的高斯先验，这些列表示定义了主子空间的响亮。每个这样的高斯分布有一个独立的方差，由精度超参数$$ \alphai $$控制，从而    
...
$$ \alpha_i $$的值可以通过最大化边缘似然函数的方式迭代地求解，其中$$ W $$被积分出去。作为最优化的结果，某个$$ \alphai $$可能趋于无穷大，对应的参数向量$$ w_i $$趋于0（后验概率分布变成了原点处的delta函数），得到了一个稀疏解。这样，主子空间的有效的维度由有限的$$ \alphai $$的值确定，对应的向量$$ w_i
