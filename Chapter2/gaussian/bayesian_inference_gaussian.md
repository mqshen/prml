...
$$趋向与0，同时后验分布在最大似然解附近变成无限高的尖峰。因此式（2.143）给出的$$ \mu $$的最大似然的点估计可以通过贝叶斯在观测数量趋于无穷时恢复精度。注意，即使对于有限的$$ N $$，当$$ \delta_0^2 \to \infty $$即先验有无限的方差，那么式（2.141）的后验均值退化成最大似然解，而式（2.142）的后验方差由$$ \delta_N^2 = \delta^2 / N $$给出。    
